...
public class OutputProcessor extends InputProcessor{
	private static final Logger log = LogManager.getLogger(OutputProcessor.class);
	...
	/**
	 * procesa aquellas consultas que buscan el equivalente decimal de cualquier RomanNumeral e imprime el resultado.
	 * @param query
	 */
	public static void findValueOfRoman(String query){
		if (isValidinput(query)== true){
			...
			log.info(query+" "+outputFormatter(tokenValue));
		}
		else{
	...
	private static void findValueOfElement(String query){
		if (isValidinput(query) == true){
			...
			log.error(query+" : I have no idea what you are talking about");
		}
	}
